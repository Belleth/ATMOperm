# Project ATMOperm
# Gernot Resch 25.2.2019
# Run 10000 simulations of snowpack on a multicore environment and evaluate
# daily active layer thickness (alt). also save modelled depth and temperature in .RData-files
##############################################################################
...
if (hostname == "Linux"){                                                          # switch to directory if script runs on icebear
    setwd("~/atmoperm/")
    cores <- 25                                                                    # set number of parallel snowpack runs. max: 32,
...
source("02_scripts/work_files/f_pro.import.R")                                                # import extracted .pro-data
    ...
    for (x in 1:ncol(runs)){
        ...
        if (length(runs.completed) == 1){
            ...
            if (!is.na(size) &&
                ...
                angle <- system(paste0("cat input/sno/", experiments[sel],               # get angle for height-calculation of depth
                                       ".sno | grep SlopeAngle"), intern=T)
                angle <- (90 - as.numeric(gsub(                                          # isolate angle and change from degree to radians
                    angle, pattern="SlopeAngle = ", replacement=""))) / (180/pi)
                ...
                save(temp.modelled.full,                                                 # export data
                     file=paste0("../01_data/soil/sampling/temp.modelled.full.",
                ...
                save(temp.modelled,                                                      # export data
                     file=paste0("../01_data/soil/sampling/temp.modelled.", 
                                 experiments[sel], ".RData"))
                ...
                save(alt.daily, file=paste0("../01_data/soil/sampling/alt.",   # save to disk
                                            experiments[sel], ".RData"))
            }
        }
    }
...
# +++++ cleanup after all model runs have finished
