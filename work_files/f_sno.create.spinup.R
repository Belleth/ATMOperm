...
# Gernot Resch 29.05.2018
####################################################################################
# example parameters
# start <- "2010-08-26"
# soil <- "soil.temp"
# depths <- "depth.obs"
# file <- "sonnblick_spinup.sno"

####################################################################################
sno.create.spinup <- function(start, soil, depths, file){
    {
        depth.obs <- get(depths)                                     # import measurement-depths
        
        input <- read.table(paste0("output/", file), skip=25)        # import modelled sno-file-data
        nsnow <- as.numeric(strsplit(system(paste0(                  # count modelled snow layers
            "cat output/", file, " | grep nSnowLayer"),
            intern=T), "=")[[1]][2])
        nsoil <- nrow(input) - nsnow                                 # reduce to soil layers
        input <- input[1:nsoil,]                                     
        input[,1] <- as.character(input[,1])                         # convert date to character
        
        depth.modelled <- rep(NA, nrow(input))                       # calculate depth of each layer
            sel <- rev(input[,2])
            for (i in 1:length(depth.modelled)){
                depth.modelled[i] <- sum(sel[1:i])
            }    
        depth.modelled <- rev(depth.modelled)
        ...
        # search for temperature of each layer
        sel <- rep(NA, length(depth.modelled))
        
        for (i in 1:length(sel)){
            sel[i] <- min(which(depth.modelled[i] <= depth.obs))
            
        }    
            
            
        # header <- read.table(paste0("output/", file), sep="|")       # prepare header
        # header <- as.character(header[1:25,])
        # header[15] <- "nSnowLayerData   = 0"
        # 
        # output <- as.data.frame(array(NA, c(25+nrow(input), 19)))    # prepare output-file
        # output[1:25, 1] <- header
        # output[c(25+1):nrow(output),] <- input
        # 
        # write.table(output, file=paste0("input/spinup/", file),      # write sno-object to file system
